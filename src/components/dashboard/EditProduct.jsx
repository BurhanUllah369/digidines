...
const AddProduct = () => {
  ...
  const handleVariationChange = (index, e) => {
    ...
    const updatedVariations = [...variations];
    if (name === "name") {
      updatedVariations[index].menu_size.description = value;
    } else if (name === "price") {
  ...
  useEffect(() => {
    if (selectedProduct) {
      ...
      setVisible(selectedProduct.is_visible);
      setRecommended(selectedProduct.is_most_selling);
      setVariations(selectedProduct.variants);
    }
  }, [selectedProduct]);
  ...
  const handleSubmit = async (e) => {
    ...
    const variationsModified = variations.some((variation, index) => {
      return (
        variation.menu_size.description !==
          selectedProduct.variants[index].menu_size.description ||
        variation.price !== selectedProduct.variants[index].price
      );
    });
    ...
    const otherFieldsModified =
      ...
      visible !== selectedProduct.is_visible ||
      recommended !== selectedProduct.is_recommended ||
      image !== "";

    if (!variationsModified && !otherFieldsModified) {
      return;
    }

    // Proceed with submitting modified data
    const token = localStorage.getItem("accessToken");
    ...
    const allVariations = variations.map((variation) => ({
      menu_size: {
        description: variation.menu_size.description,
      },
    ...
    try {
      const response = await axios.put(
        `${API_ENDPOINTS.UPDATE_MENU_ITEM}${selectedProduct.id}/`,
        formData,
        {
          headers: {
            Authorization: `Bearer ${token}`,
        ...
        {variations.map((variation, index) => (
          ...
          >
            <input
              ...
              name="name"
              value={variation.menu_size.description}
              required
